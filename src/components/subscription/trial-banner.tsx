...
export function TrialBanner({ variant = 'full' }: TrialBannerProps) {
  ...
  if (variant === 'compact' || (isBasicNonUrgent && !isUrgent)) {
    return (
      <Link
        href="/suscripcion"
        className="mb-4 mt-1 flex w-full items-center justify-between gap-3 rounded-2xl border border-zinc-200/60 bg-white/70 px-3.5 py-3 shadow-[0_8px_24px_rgba(0,0,0,0.08)] backdrop-blur-xl transition-colors hover:bg-zinc-50/90 dark:border-white/10 dark:bg-black/25 dark:hover:bg-black/35"
      >
