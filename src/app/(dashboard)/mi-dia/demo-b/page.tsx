...
/**
 * DEMO B: Split Dashboard Pro
 *
 * Awwwards Score: 7/10
 *
 * Features:
 * - Left panel: Timeline with all appointments
 * - Center panel: Selected appointment details
 * - Right panel: Quick stats and actions
 * - Inline editing (simulated)
 * - Keyboard shortcuts (j/k navigation)
 * - Desktop-optimized power user UX
 */
export default function MiDiaDemoBPage() {
  ...
  useEffect(() => {
    if (!data?.appointments || isEditing) return
  ...
  }, [data?.appointments, selectedAppointmentId, isEditing])

  // Auto-select first appointment on load
  useEffect(() => {
    if (data?.appointments && data.appointments.length > 0 && !selectedAppointmentId) {
  ...
  // Error
  if (fetchError && !data) {
    return <ErrorState message={fetchError.message} onRetry={refetch} buttonText="Reintentar" />
  }
          ...
          <AnimatePresence mode="wait">
            {selectedAppointment ? (
              <AppointmentDetailsPanel
                ...
                onComplete={complete}
                onNoShow={noShow}
                isLoading={loadingAppointmentId === selectedAppointment.id}
              />
            ) : (
              <motion.div
                initial={{ opacity: 0 }}
              ...
              <div>
                ...
                <div className="space-y-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => checkIn(selectedAppointment.id)}
                    disabled={
                      selectedAppointment.status !== 'pending' ||
                      loadingAppointmentId === selectedAppointment.id
                    }
                  ...
                  <Button
                    variant="success"
                    size="sm"
                    onClick={() => complete(selectedAppointment.id)}
                    disabled={loadingAppointmentId === selectedAppointment.id}
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => noShow(selectedAppointment.id)}
                    disabled={loadingAppointmentId === selectedAppointment.id}
  ...
  return (
    <div className="h-screen bg-zinc-950 flex">
      <div className="w-80 bg-zinc-900 border-r border-zinc-800 p-4 space-y-2 animate-pulse">
        <div className="h-10 bg-zinc-800 rounded-lg mb-4" />
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-24 bg-zinc-800 rounded-lg" />
        ))}
      </div>
      <div className="flex-1 p-8 animate-pulse">
        <div className="h-8 bg-zinc-800 rounded-lg w-64 mb-8" />
      ...
      <div className="w-72 bg-zinc-900 border-l border-zinc-800 p-6 space-y-3 animate-pulse">
        {[...Array(5)].map((_, i) => (
